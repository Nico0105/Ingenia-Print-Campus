...
// Extrae secciones: materiales compatibles e ideal para
function getSecciones(contenido) {
  ...
  if (Array.isArray(contenido.materiales_compatibles)) {
    secciones.push({
      titulo: 'Materiales Compatibles',
      items: contenido.materiales_compatibles,
    });
  }

  // Ideal para
  if (Array.isArray(contenido.ideal_para)) {
    secciones.push({
      titulo: 'Ideal Para',
      items: contenido.ideal_para,
    });
  }
...
<<<<<<< HEAD
      ...
      <section className="product-specs">
        ...
        {/* Especificaciones Técnicas full width */}
        {product.contenido?.especificaciones && Object.keys(product.contenido.especificaciones).length > 0 && (
          <div className={`spec-section full-width ${Object.keys(product.contenido.especificaciones).length <= 3 ? 'compact' : ''}`}>
        ...
        {/* Otras secciones en grid de 2 columnas */}
        {secciones.length > 0 && (
          <div className="specs-sections-grid">
            {secciones.map((seccion, i) => (
              <div className={`spec-section ${seccion.items.length <= 2 ? 'compact' : ''}`} key={i}>
